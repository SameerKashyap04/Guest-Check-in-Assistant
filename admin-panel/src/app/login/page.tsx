  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setIsLoading(false);
        setErrorMsg(data.error || "Invalid Super-Admin email or password.");
      }
    } catch (err: any) {
      setIsLoading(false);
      ...
      <div className="bg-white border border-slate-200/90 rounded-3xl p-8 max-w-md w-full shadow-2xl">
        <div className="items-center text-center mb-6">
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Access strictly restricted to authorized Super Administrators
          </p>
        </div>

        {errorMsg && (
          <div className="mb-5 p-3.5 rounded-2xl bg-rose-50 border border-rose-200 flex items-start gap-2.5 text-xs font-semibold text-rose-800 animate-in fade-in">
