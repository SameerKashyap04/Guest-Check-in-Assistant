...
export function PrimaryButton({label, onPress, icon, style}: {label: string; onPress?: ()=>void; icon?: IconName; style?: ViewStyle}) {
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={onPress} style={[s.primary, style]}>
...
export function SecondaryButton({label, onPress, icon, style, textStyle}: {label: string; onPress?: ()=>void; icon?: IconName; style?: ViewStyle; textStyle?: TextStyle}) {
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={onPress} style={[s.secondary, style]}>
      {icon && <Icon name={icon} size={17} color={(style as any)?.borderWidth === 0 ? '#fff' : C.ink}/>}
      <Text style={[s.secondaryText, textStyle, (style as any)?.borderWidth === 0 && {color: '#fff'}]}>{label}</Text>
    </TouchableOpacity>
  );
}
...
export function SoftButton({label, onPress, icon, style}: {label: string; onPress?: ()=>void; icon?: IconName; style?: ViewStyle}) {
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={onPress} style={[s.soft, style]}>
...
export function SettingRow({icon, label, subtitle, onPress, right}: {icon: IconName; label: string; subtitle?: string; onPress?: ()=>void; right?: React.ReactNode}) {
  return (
